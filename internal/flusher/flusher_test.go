...
var _ = Describe("Flusher.Flush(chunkSize, repo)", func() {
	...
	Context("when repo working correct", func() {
		...
		Context("when chunkSize greater than zero", func() {
			BeforeEach(func() {
				...
				mockRepo.EXPECT().AddTasks(gomock.Any(), gomock.Any()).DoAndReturn(func(ctx context.Context, entities []tasks.Task) error {
	...
	Context("when repo working with errors ", func() {
		...
		Context("when repo cant add just second chunk", func() {
			BeforeEach(func() {
				var callCounter = 0
				mockRepo.EXPECT().AddTasks(gomock.Any(), gomock.Any()).
					DoAndReturn(func(ctx context.Context, entities []tasks.Task) error {
						callCounter++
						if callCounter == 2 {
							db = append(db, entities...)
							return nil
